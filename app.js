...
for(var i=0; i<parkInfo.length; i++){
  var select = `<option data-toggle="modal" data-target="#${parkInfo[i].id}">${parkInfo[i].title}</option>`;
  $('#park').append(select);
  var park =
  `<div id="${parkInfo[i].id}" class="modal fade" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <img class="img-responsive" src="${parkInfo[i].image}" alt="national park">
          <h1 class="modal-title">${parkInfo[i].title}</h1>
          <h2 class="modal-title">Est. ${parkInfo[i].year}</h2>
        </div>
        <div class="modal-body">
          <h3>Top Features: </h3>
          <p>${parkInfo[i].features}</p>
...
function setHawaiiMarkers(hawaiiMap) {
  var hawaiiMarkers = [];
  for (var i = 0; i < parkInfo.length; i++) {
    if(parkInfo[i].map === 'hawaiiMap') {
      var marker = new google.maps.Marker ({
        position: parkInfo[i].position,
        map: hawaiiMap,
        title: parkInfo[i].title,
        popup: '#' + parkInfo[i].id,
